class UsersController < ApplicationController
  ...
  def show
    ...
    unless @offering_ids.empty?
      @active_offerings = Offering.where('id IN (?) AND (offerings.courseDate > ? OR (offerings.courseDate = ? AND offerings.courseTime >= ?))', @offering_ids, Date.today, Date.today, Time.now.strftime("2000-01-01 %H:%M:00"))
      @past_offerings = Offering.where('id IN (?) AND (offerings.courseDate < ? OR (offerings.courseDate = ? AND offerings.courseTime < ?))', @offering_ids, Date.today, Date.today, Time.now.strftime("2000-01-01 %H:%M:00"))
    end
  end
